...
const { syntaxResponse, formatResponse } = require('../../Util/Embeds/Others/manga')
...
module.exports = new Command({
	...
	async run(message, args, client) {
        ...
        function handleData(data) {
            ...
            const url = `https://anilist.co/mang/${id}`
            ...
            let chapters = dat.chapters
            let volumes = String(dat.volumes)
            ...
            if (!chapters) {
                ...
                chapters = String(dat.chapters)
                endDate = `${dat.endDate.year}-${dat.endDate.month}-${dat.endDate.day}`
            }
            const date = `From **${startDate}** to **${endDate}**`
       //     console.log(chapters,volumes, date)
            message.channel.send({ embeds : [formatResponse(title_english, url, description, status, genres,date,chapters,volumes, averageScore, thumbnail,color)] })
        }
